...
function showResult() {
  ...
  if (userScore > 3) {
    scoreTag = `<span>y felicidades! 🎉, obtuviste <p>${userScore}</p> de <p>${questions.length}</p></span>`;
  } else if (userScore > 1) {
    scoreTag = `<span>y bien hecho 😎, obtuviste <p>${userScore}</p> de <p>${questions.length}</p></span>`;
  } else {
    scoreTag = `<span>lo siento 😐, solo obtuviste <p>${userScore}</p> de <p>${questions.length}</p></span>`;
  }
